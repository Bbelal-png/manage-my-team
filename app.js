...
// ═══════════════════════════════════
const WIZARD_KEY = 'mmt_wizard_done';
let wizStep = 0;
let wizData = {
  clubName:'', city:'', emoji:'⚽', logoUrl:null,
  season:'', league:'',
  coachName:'', coachPhone:'', presidentName:'',
  teams:[{name:'Équipe A',icon:'🟢',color:'#00E676'}],
  numTeams:1
};

const WIZARD_STEPS = [
  {title:'BIENVENUE',        subtitle:'Configurons votre club en 5 étapes', next:'Commencer →'},
  {title:'VOTRE CLUB',       subtitle:'Informations générales',              next:'Suivant →'},
  {title:'LA SAISON',        subtitle:'Compétition et calendrier',           next:'Suivant →'},
  {title:'LE STAFF',         subtitle:'Encadrement technique',               next:'Suivant →'},
  {title:'VOS ÉQUIPES',      subtitle:'Nombre et nom des équipes',           next:'Suivant →'},
  {title:"C'EST PARTI !",    subtitle:'Récapitulatif de votre club',         next:"🚀 Lancer l'application"}
];

function wizIsFirstLaunch() {
  try {
    // Si pas de storage disponible, montrer quand même le wizard
    if (!HAS_STORAGE) return true;
    // Si wizard déjà complété, ne pas montrer
    if (localStorage.getItem(WIZARD_KEY)) return false;
    // Si données sauvegardées avec un vrai nom de club, ne pas montrer
    const raw = localStorage.getItem(STORAGE_KEY);
    if (raw) {
      const data = JSON.parse(raw);
      // Si le club s'appelle encore "FC Étoile" c'est les données de démo
      if (data.TEAM && data.TEAM.name && data.TEAM.name !== 'FC Étoile 2024') return false;
    }
    return true;
  } catch(e) { return true; }
}
...
function wizRender() {
  ...
  document.getElementById('wiz-steps').innerHTML = WIZARD_STEPS.map((_,i) =>
    `<div class="wizard-step-dot ${i<wizStep?'done':i===wizStep?'active':''}"></div>`
  ).join('');
  document.getElementById('wiz-title').textContent = step.title;
  document.getElementById('wiz-subtitle').textContent = step.subtitle;
  document.getElementById('wiz-next').textContent = step.next;
  document.getElementById('wiz-back').style.display = wizStep===0?'none':'block';

  const body = document.getElementById('wiz-body');
  const EMOJIS = ['⚽','🏆','🦁','🐯','🦅','🐺','⭐','🔥','💎','🛡️','⚡','🌟','🏅','🎯','🦊','🐉'];
  const teamIcons = ['🟢','🔵','🔴']; const teamColors = ['#00E676','#4DABF7','#FF4757'];

  if (wizStep===0) {
    body.innerHTML = `
      <div style="text-align:center;padding:1rem 0">
        <div style="font-size:4rem;margin-bottom:1rem">⚽</div>
        <div style="font-family:'Bebas Neue',sans-serif;font-size:1.8rem;color:var(--green);letter-spacing:.05em">ManageMyTeam</div>
        <div style="font-size:.88rem;color:var(--muted);margin-top:.5rem;line-height:1.7">L'application de gestion pour clubs de football amateur.<br>Configurons ensemble votre club en 2 minutes.</div>
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:.6rem;margin-top:1.5rem">
          ${[['👥','Joueurs & Staff'],['📊','Stats & Résultats'],['📱','Mobile Ready'],['💶','Finances'],['⚙️','Tactique'],['📄','Export PDF']].map(([i,l])=>`<div style="background:var(--card);border:1px solid var(--border);border-radius:8px;padding:.6rem .4rem;font-size:.68rem;color:var(--muted);text-align:center"><div style="font-size:1.2rem;margin-bottom:.2rem">${i}</div>${l}</div>`).join('')}
        </div>
      </div>`;
  } else if (wizStep===1) {
    body.innerHTML = `
      <div><div class="wiz-label">Nom du club *</div>
      <input class="wiz-input" id="w-club" placeholder="Ex: FC Étoile, US Montceau..." value="${wizData.clubName}" oninput="wizData.clubName=this.value" /></div>
      <div><div class="wiz-label">Ville</div>
      <input class="wiz-input" id="w-city" placeholder="Ex: Lyon, Paris..." value="${wizData.city}" oninput="wizData.city=this.value" /></div>
      <div><div class="wiz-label">Emoji du club</div>
      <div class="wiz-emoji-grid">${EMOJIS.map(e=>`<button class="wiz-emoji-btn ${wizData.emoji===e?'active':''}" onclick="wizSetEmoji('${e}',this)">${e}</button>`).join('')}</div></div>`;
  } else if (wizStep===2) {
    const yr = new Date().getFullYear();
    body.innerHTML = `
      <div><div class="wiz-label">Saison *</div>
      <input class="wiz-input" id="w-season" placeholder="Ex: ${yr}-${yr+1}" value="${wizData.season||(yr+'-'+(yr+1))}" oninput="wizData.season=this.value" /></div>
      <div><div class="wiz-label">Compétition / Ligue</div>
      <input class="wiz-input" id="w-league" placeholder="Ex: Division Régionale 3..." value="${wizData.league}" oninput="wizData.league=this.value" /></div>
      <div style="background:var(--card);border:1px solid var(--border);border-radius:8px;padding:.75rem;font-size:.75rem;color:var(--muted)">
        💡 Modifiable depuis <strong style="color:var(--text)">Mon Équipe → Modifier</strong></div>`;
  } else if (wizStep===3) {
    body.innerHTML = `
      <div class="wiz-row">
        <div><div class="wiz-label">Prénom entraîneur</div>
        <input class="wiz-input" id="w-coach-f" placeholder="Prénom" value="${wizData.coachName.split(' ')[0]||''}" /></div>
        <div><div class="wiz-label">Nom</div>
        <input class="wiz-input" id="w-coach-l" placeholder="Nom" value="${wizData.coachName.split(' ').slice(1).join(' ')||''}" /></div>
      </div>
      <div><div class="wiz-label">Téléphone (optionnel)</div>
      <input class="wiz-input" id="w-coach-tel" type="tel" placeholder="06 XX XX XX XX" value="${wizData.coachPhone}" oninput="wizData.coachPhone=this.value" /></div>
      <div><div class="wiz-label">Président / Dirigeant (optionnel)</div>
      <input class="wiz-input" id="w-president" placeholder="Prénom Nom" value="${wizData.presidentName}" oninput="wizData.presidentName=this.value" /></div>`;
  } else if (wizStep===4) {
    body.innerHTML = `
      <div><div class="wiz-label">Nombre d'équipes</div>
      <div style="display:flex;gap:.5rem">
        ${[1,2,3].map(n=>`<button onclick="wizSetTeams(${n})" style="flex:1;padding:.6rem;border-radius:8px;border:1.5px solid ${wizData.numTeams===n?'var(--green)':'var(--border)'};background:${wizData.numTeams===n?'rgba(0,230,118,.08)':'var(--card)'};color:${wizData.numTeams===n?'var(--green)':'var(--muted)'};cursor:pointer;font-weight:700;font-size:.88rem">${n} équipe${n>1?'s':''}</button>`).join('')}
      </div></div>
      <div id="wiz-teams-list">
        ${Array.from({length:wizData.numTeams},(_,i)=>`
          <div class="wiz-team-row">
            <div class="wiz-team-badge" style="background:${teamColors[i]}22;color:${teamColors[i]}">${teamIcons[i]}</div>
            <input class="wiz-input" style="flex:1;background:transparent;border:none;padding:.2rem .4rem" placeholder="Nom de l'équipe" value="${wizData.teams[i]?.name||'Équipe '+String.fromCharCode(65+i)}" oninput="if(!wizData.teams[${i}])wizData.teams[${i}]={};wizData.teams[${i}].name=this.value" />
          </div>`).join('')}
      </div>`;
  } else if (wizStep===5) {
    body.innerHTML = `
      <div style="text-align:center;margin-bottom:.5rem">
        <div style="font-size:3rem">${wizData.emoji}</div>
        <div style="font-family:'Bebas Neue',sans-serif;font-size:1.4rem;color:var(--green);letter-spacing:.05em">${wizData.clubName||'Mon Club'}</div>
        <div style="font-size:.78rem;color:var(--muted)">${wizData.city||''}</div>
      </div>
      <div style="background:var(--card);border:1px solid var(--border);border-radius:10px;overflow:hidden">
        ${[['🏆 Saison',wizData.season||'—'],['⚽ Compétition',wizData.league||'Non renseignée'],['🏋️ Entraîneur',wizData.coachName.trim()||'Non renseigné'],['👔 Président',wizData.presidentName.trim()||'Non renseigné'],['🏟️ Équipes',wizData.teams.slice(0,wizData.numTeams).map(t=>t.name).join(', ')]].map(([l,v])=>`
          <div style="display:flex;justify-content:space-between;padding:.5rem .9rem;border-bottom:1px solid var(--border);font-size:.82rem">
            <span style="color:var(--muted)">${l}</span><span style="font-weight:700">${v}</span>
          </div>`).join('')}
      </div>
      <div style="font-size:.75rem;color:var(--muted);text-align:center">✏️ Tout est modifiable depuis l'application</div>`;
  }
}

function wizSetEmoji(e, el) {
  wizData.emoji = e;
  document.querySelectorAll('.wiz-emoji-btn').forEach(b=>b.classList.remove('active'));
  el.classList.add('active');
}

function wizSetTeams(n) {
  const icons=['🟢','🔵','🔴']; const colors=['#00E676','#4DABF7','#FF4757'];
  wizData.numTeams = n;
  wizData.teams = Array.from({length:n},(_,i)=>wizData.teams[i]||{name:'Équipe '+String.fromCharCode(65+i),icon:icons[i],color:colors[i]});
  wizRender();
}

function wizCollectInputs() {
  const map = {'w-club':'clubName','w-city':'city','w-season':'season','w-league':'league','w-coach-tel':'coachPhone','w-president':'presidentName'};
  Object.entries(map).forEach(([id,key])=>{ const el=document.getElementById(id); if(el) wizData[key]=el.value; });
  const cf=document.getElementById('w-coach-f'), cl=document.getElementById('w-coach-l');
  if(cf&&cl) wizData.coachName=(cf.value+' '+cl.value).trim();
  document.querySelectorAll('#wiz-teams-list input').forEach((el,i)=>{ if(wizData.teams[i]) wizData.teams[i].name=el.value||wizData.teams[i].name; });
}

function wizNext() {
  wizCollectInputs();
  if (wizStep===1 && !wizData.clubName.trim()) {
    const inp=document.getElementById('w-club');
    if(inp){inp.style.borderColor='var(--red)';inp.focus();}
    showToast('⚠️ Le nom du club est requis'); return;
  }
  if (wizStep < WIZARD_STEPS.length-1) { wizStep++; wizRender(); }
  else wizFinish();
}
...
function wizFinish() {
  wizCollectInputs();
  const clubName = wizData.clubName.trim()||'Mon Club';
  const season   = wizData.season.trim()||(new Date().getFullYear()+'-'+(new Date().getFullYear()+1));

  // Apply club info
  TEAM.name=clubName; TEAM.city=wizData.city||''; TEAM.league=wizData.league||'';
  TEAM.season='Saison '+season; TEAM.emoji=wizData.emoji; TEAM.logoUrl=null;
  CLUB.name=clubName; CLUB.city=wizData.city||''; CLUB.emoji=wizData.emoji;
  CLUB.league=wizData.league||''; CLUB.season='Saison '+season;
  SEASONS.length=0; SEASONS.push({id:1,label:season,active:true});

  // Apply staff
  if(wizData.coachName.trim()) { STAFF[0].name=wizData.coachName.trim(); STAFF[0].phone=wizData.coachPhone||''; }
  if(wizData.presidentName.trim()) STAFF[4].name=wizData.presidentName.trim();

  // Apply teams
  SUB_TEAMS.length=0;
  wizData.teams.slice(0,wizData.numTeams).forEach((t,i)=>{
    SUB_TEAMS.push({id:String.fromCharCode(65+i),name:t.name,icon:t.icon,color:t.color,playerIds:[]});
  });

  // Clear demo data — start fresh
  PLAYERS.length=0; EVENTS.length=0; SESSIONS.length=0;
  Object.keys(ATTENDANCE).forEach(k=>delete ATTENDANCE[k]);
  INJURIES.length=0; NEWS.length=0; PROJECTS.length=0; FIN_TRANSACTIONS.length=0;

  // Mark wizard done
  try { if(HAS_STORAGE) localStorage.setItem(WIZARD_KEY,'1'); } catch(e){}

  // Hide wizard & launch app
  document.getElementById('wizard-overlay').style.display='none';
  renderStats(); renderCalendar(); renderResults(); renderAttendance();
  renderPlayers(); renderTeam(); updatePlanUI(); navigate('home');
  setTimeout(saveAll,500);
  showToast('🎉 Bienvenue '+clubName+' !');
}
...
window.addEventListener('load', () => {
  ...
  if (wizIsFirstLaunch()) {
  ...
  } else if (loadResult && loadResult.loaded) {
    ...
    setTimeout(()=>showToast('💾 Données restaurées du '+when+(loadResult.noPhotos?' (sans photos)':'')),600);
  }

  if (!loadResult && HAS_STORAGE) setTimeout(saveAll,1000);
  ...
  document.addEventListener('click', e => {
    ...
    if(!e.target.closest('#bn-more')&&!e.target.closest('#more-menu')) closeMoreMenu();
  });

  if (HAS_STORAGE) window.addEventListener('beforeunload', saveAll);
});
